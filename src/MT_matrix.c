 ...
 * MT_matrix.c
 * 
 * 多執行緒矩陣相乘程式（Multi-threaded Matrix Multiplication）
...
static Matrix *m1, *m2, *m;                   // 矩陣指標：m1, m2 為輸入，m 為結果
 ...
 * 展示動態記憶體配置（dynamic memory allocation）的使用
 * 
...
Matrix* load_matric(char* filename)
{
    ...
    // 為二維陣列配置記憶體（two-level dynamic allocation）
    ...
    fclose(fp);  // 關閉檔案，釋放檔案描述符（file descriptor）
    return m;
}

/*
 * thread() - Thread 執行函式（Thread Routine）
 * 
 ...
 * 展示檔案 I/O 操作和資料持久化（data persistence）概念
 */
void write_result()
{
...
int main(int argc, char* argv[])
{
    // 檢查命令列參數數量
    if (argc != 4) {
        printf("Usage: ./MT_matrix [number of worker threads] [file name of input matrix1] [file name of input matrix2]\n");
        return 0;
    }
