...
#include <linux/kernel.h>     // 核心基本函式和巨集
#include <linux/module.h>     // 核心模組相關函式和巨集
...
/*
 * Linux 核心版本相容性處理
 * 核心 5.6.0 之後，proc_ops 取代了 file_operations 結構
 */
 ...
 * 
 * 當 user space application 寫入資料到 /proc/thread_info 時，核心會呼叫此函式
 * 這是此模組的核心功能，展示以下重要概念：
 * 1. Userspace-to-kernel data transfer
 * 2. Process table traversal（程序表遍歷）
...
/*
 * 檔案操作結構體定義
 * 將我們自定義的讀寫函式與 /proc filesystem 連接
 * 
 * Linux 核心版本相容性處理：
 ...
 * 
 * 當模組卸載時（使用 rmmod 命令）核心會呼叫此函式
 * 負責清理資源，移除 /proc/thread_info 檔案
 * 
 * __exit 標記表示此函式只在模組卸載時使用，
 * 如果模組被編譯到 kernel 中（非模組形式），此函式會被忽略
 */
